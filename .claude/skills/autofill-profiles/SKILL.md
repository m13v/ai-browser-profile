...
-- Addresses
SELECT name, value, count FROM autofill WHERE lower(name) IN ('address', 'street', 'city', 'state', 'zip', 'zipcode', 'postal', 'country', 'address1', 'address2') ORDER BY count DESC;

-- Companies
SELECT value, count FROM autofill WHERE lower(name) IN ('company', 'companyname', 'company_name', 'organization', 'org') ORDER BY count DESC;
```
...
# Usage:
# for browser, profile, path in find_all_webdata():
#     profiles = extract_address_profiles(path)
#     print(f"{browser}/{profile}: {len(profiles)} address profiles")
```

## What This Tells You About a Person

A single browser's autofill data typically reveals:
- **Full legal name** (from type 7, or types 3+5)
- **All email addresses** (type 9 + autofill `email` entries — often multiple: personal, work, aliases)
- **Phone number** (type 14)
- **Home/work/shipping addresses** (types 77, 33-36 — multiple profiles = multiple addresses)
- **Company/employer** (type 60)
- **Country and region** (types 34, 36)
- **Synced vs local** profiles — `record_type=1` means synced from Google account (even more authoritative)
- **Usage patterns** — `use_count` shows which address profile is primary

Combined with the `autofill` table form entries, you can also find:
- Usernames and login identifiers
- Business-specific form data (industry-specific fields)
- Frequently used values for any web form
